...
class StoreService {
    ...
    Payment purchaseAlbum(User user, creditCard, List albumPayments) {
      def p = new Payment(
         user: user,
         invoiceNumber: "INV-${user.id}-${System.currentTimeMillis()}"
      )
      ...
      for (ap in albumPayments) {
         ap.user = user
         // validation should never fail at this moment
         if (!ap.validate()) {
